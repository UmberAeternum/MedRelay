...
describe("safe pipeline order and provider behavior", () => {
  ...
  it("rejects model diagnosis output", async () => { const provider:ModelProvider={configured:true,model:"gpt-5.6",reply:vi.fn(async messages=>({...validReply(messages),message:"The diagnosis is flu"})),handoff:vi.fn()}; const x=await setup(provider); await expect(x.service.continue(x.conversationId,x.accessToken,"owner","fever")).rejects.toThrow("UNSAFE_MODEL_OUTPUT"); });
  it("rejects invalid model evidence IDs", async () => { const provider:ModelProvider={configured:true,model:"gpt-5.6",reply:vi.fn(async messages=>({...validReply(messages),evidence:[{...validReply(messages).evidence[0],sourceMessageId:"10000000-0000-4000-8000-000000000000"}]})),handoff:vi.fn()}; const x=await setup(provider); await expect(x.service.continue(x.conversationId,x.accessToken,"owner","fever")).rejects.toThrow("INVALID_EVIDENCE"); });
  it("rejects deterministic urgency downgrade", async () => { const provider:ModelProvider={configured:true,model:"gpt-5.6",reply:vi.fn(async messages=>validReply(messages)),handoff:vi.fn()}; const x=await setup(provider); await expect(x.service.continue(x.conversationId,x.accessToken,"owner","headache with vomiting")).rejects.toThrow("URGENCY_DOWNGRADE"); });
