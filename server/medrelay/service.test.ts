...
const OFFLINE_MESSAGE = "MedRelay captured your statement and prepared an editable clinician-review draft. Continue with the focused follow-up below.";
...
describe("safe pipeline order and provider behavior", () => {
  ...
  it("returns the exact honest offline draft message without a key", async () => { const x=await setup(); const result=await x.service.continue(x.conversationId,x.accessToken,"owner","headache"); expect(result.providerMode).toBe("offline"); expect(result.reply.message).toBe(OFFLINE_MESSAGE); });
  ...
  it("falls back to the exact offline draft message when a provider response is rejected", async () => { const provider:ModelProvider={configured:true,model:"gpt-5.6",reply:vi.fn(async()=>{ throw new Error("OPENAI_RESPONSE_FAILED"); }),handoff:vi.fn()}; const x=await setup(provider); const result=await x.service.continue(x.conversationId,x.accessToken,"owner","headache"); expect(result.providerMode).toBe("offline"); expect(result.reply.message).toBe(OFFLINE_MESSAGE); });
