...
describe("structured output consistency", () => {
  ...
  it("requires a focused question for information gaps", () => expect(ConversationReplySchema.safeParse({...baseReply,followUpQuestion:null}).success).toBe(false));
  it("does not allow a question during emergency guidance", () => expect(ConversationReplySchema.safeParse({...baseReply,careLevel:"emergency_services",warningSigns:["warning"],emergencyGuidance:"Seek care",followUpQuestion:"What happened?",informationGaps:[]}).success).toBe(false));
